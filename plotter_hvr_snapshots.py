...
for instance in instances:
    for solver in solvers:
        for seed in seeds:
            filename = os.path.join(dirname, "hvr_snapshots/" + instance + "_" + solver + "_" + str(seed) + ".txt")
            if os.path.exists(filename):
                with open(filename) as csv_file:
                    data = csv.reader(csv_file, delimiter = ",")
                    for row in data:
                        min_hvr = min(min_hvr, float(row[2]))
                        max_hvr = max(max_hvr, float(row[2]))
...
for solver in solvers:
    ...
    for i in range(num_snapshots):
        hvr_per_solver[solver].append([])
        time_per_solver[solver].append([])

hvr_per_snapshot = []

for i in range(num_snapshots):
    hvr_per_snapshot.append([])
    for solver in solvers:
        hvr_per_snapshot[i].append([])
            ...
            if os.path.exists(filename):
                with open(filename) as csv_file:
                    ...
                    for row in data:
                        ...
                        hvr_per_solver[solvers[i]][j].append(float(row[2]))
                        hvr_per_snapshot[j][i].append(float(row[2]))
                        j += 1
                    csv_file.close()
...
for group in solver_groups:
    ...
    for i in indices:
        ...
        for j in range(num_snapshots):
            ...
            y.append(stats.mean(hvr_per_solver[solvers[i]][j]))
            z.append(stats.stdev(hvr_per_solver[solvers[i]][j]))
        ...
        for solver in solvers:
            for j in range(num_snapshots):
                ...
                data['Hypervolume Ratio'].append(stats.mean(hvr_per_solver[solver][j]))
                data['Standard Deviation'].append(stats.stdev(hvr_per_solver[solver][j]))

        df = pd.DataFrame(data)

        # Save the DataFrame to a CSV file
        csv_filename = os.path.join(dirname, "hvr_snapshots/hvr_data.csv")
        df.to_csv("hvr_data.csv", index=False)
    ...
    plt.tight_layout()
    filename = os.path.join(dirname, "hvr_snapshots/hvr_mean_snapshots_" + group + ".png")
    plt.savefig(bbox_inches='tight', fname=filename, format="png")
    filename = os.path.join(dirname, "hvr_snapshots/hvr_mean_snapshots_" + group + ".pdf")
...
for group in solver_groups:
    ...
    for i in indices:
        ...
        for j in range(num_snapshots):
            x.append(stats.mean(time_per_solver[solvers[i]][j]))
            quantiles = stats.quantiles(hvr_per_solver[solvers[i]][j])
    ...
    plt.tight_layout()
    filename = os.path.join(dirname, "hvr_snapshots/hvr_quartiles_snapshots_" + group + ".png")
    plt.savefig(bbox_inches='tight', fname=filename, format="png")
    filename = os.path.join(dirname, "hvr_snapshots/hvr_quartiles_snapshots_" + group + ".pdf")
    plt.savefig(bbox_inches='tight', fname=filename, format="pdf")
    plt.close()
...
for group in solver_groups:
    ...
    for snapshot in range(num_snapshots):
        ...
        with sns.color_palette([colors[i] for i in indices]):
            pt.half_violinplot(data = [hvr_per_snapshot[snapshot][i] for i in indices], palette = [colors[i] for i in indices], orient = "h", width = 0.6, cut = 0.0, inner = None)
        sns.stripplot(data = [hvr_per_snapshot[snapshot][i] for i in indices], palette = [colors[i] for i in indices], orient = "h", size = 2, zorder = 0)
        sns.boxplot(data = [hvr_per_snapshot[snapshot][i] for i in indices], orient = "h", width = 0.20, color = "black", zorder = 10, showcaps = True, boxprops = {'facecolor' : 'none', "zorder" : 10}, showfliers = True, whiskerprops = {'linewidth' : 2, "zorder" : 10}, flierprops = {'markersize' : 2})
        ...
        filename = os.path.join(dirname, "hvr_snapshots/snapshot_" + group + "_" + str(snapshot) + ".png")
