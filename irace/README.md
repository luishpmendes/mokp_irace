...
Run all six stages (plus all other solvers) in parallel, then the single
validation pass:
```bash
chmod +x irace/*-tunner*.sh irace/irace_runner.sh
./irace/irace_runner.sh
```
`irace_runner.sh` changes to its own directory first, so it works from any
current directory.
...
```bash
cd irace
TIME_LIMIT=1 Rscript -e "library(irace); irace::irace_cmdline(c('--check','--scenario','nsbrkga-scenario-stage1.txt'))"
```
...
- **`irace_runner.sh`** — Orchestration script: parallel tuning, then, only if
  every tuning succeeded, one parallel validation pass
...
- **`<instance_path>`** is formed as `trainInstancesDir/instance_name` (e.g., `../instances/zlt_100_2.txt`)
...
the 330 s cap can be raised. iRace's own per-iteration overhead (model update,
I/O) is minutes per scenario and fits inside the realistic margin.

What makes the realistic figure hold is that the solver stops itself at 300 s,
rejected NSGA-III configurations return `Inf` immediately, and iRace eliminates
weak configurations long before spending the full budget on them.
...
| iRace reports non-numeric output | The runner must print exactly one number. Try it by hand: `TIME_LIMIT=1 ./nsga2-tunner.sh 1 1 1234 ../instances/zlt_100_2.txt` |
