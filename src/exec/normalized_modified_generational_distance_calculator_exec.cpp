       ...
       arg_parser.option_exists("--reference-pareto")) {
        ...
        if(ifs.is_open()) {
            for(std::string line; std::getline(ifs, line);) {
                ...
                std::vector<double> value(instance.num_dimensions, 0.0);
            ...
            if(arg_parser.option_exists("--best-solutions-snapshots-" +
                ...
                for(unsigned j = 0; ; j++) {
                    ...
                    if(ifs.is_open()) {
                        ...
                        for(std::string line; std::getline(ifs, line);) {
                            std::istringstream iss(line);
                            std::vector<double> value(instance.num_dimensions,
                                                      0.0);
        ...
        std::cerr << "./normalized_modified_generational_distance_calculator_exec "
                  ...
                  << "--nigd-plus-snapshots-i <normalized_modified_inverted_generational_distance_snapshots_filename> "
                  << std::endl;
    }
